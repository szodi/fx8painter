...
public class CoonsPatchCreator {
	...
	public CoonsPatchCreator(ControlPoint leftTop, ControlPoint rightTop, ControlPoint leftBottom,
			ControlPoint rightBottom) {
	...
	private MutablePoint3D c1(double s) {
		return Tools.getBezierPoint(leftBottom, leftBottom.getTangent(rightBottom), rightBottom.getTangent(leftBottom),
				rightBottom, s);
	}

	private MutablePoint3D d0(double t) {
		return Tools.getBezierPoint(leftTop, leftTop.getTangent(leftBottom), leftBottom.getTangent(leftTop), leftBottom,
				t);
	}

	private MutablePoint3D d1(double t) {
		return Tools.getBezierPoint(rightTop, rightTop.getTangent(rightBottom), rightBottom.getTangent(rightTop),
				rightBottom, t);
	}
	 ...
	 * Coons patch
	 * 
	 * Bilinear blending[edit] Given four space curves c0(s), c1(s), d0(t), d1(t)
	 * which meet at four corners c0(0) = d0(0), c0(1) = d1(0), c1(0) = d0(1), c1(1)
	 * = d1(1); linear interpolation can be used to interpolate between c0 and c1,
	 * that is
	 *
	 ...
	 * 
	 * producing two ruled surfaces defined on the unit square. The bilinear
	 * interpolation on the four corner points is another surface
	 * 
	...
	private MutablePoint3D getCoonsPoint(double s, double t) {
		...
		double d_B_x = c0(0.0).getX() * (1 - s) * (1 - t) + c0(1.0).getX() * s * (1 - t) + c1(0.0).getX() * (1 - s) * t
				+ c1(1.0).getX() * s * t;
		double d_B_y = c0(0.0).getY() * (1 - s) * (1 - t) + c0(1.0).getY() * s * (1 - t) + c1(0.0).getY() * (1 - s) * t
				+ c1(1.0).getY() * s * t;
		double d_B_z = c0(0.0).getZ() * (1 - s) * (1 - t) + c0(1.0).getZ() * s * (1 - t) + c1(0.0).getZ() * (1 - s) * t
				+ c1(1.0).getZ() * s * t;
