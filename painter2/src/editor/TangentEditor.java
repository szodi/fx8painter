...
public class TangentEditor extends AbstractEditor {
	...
	public void handle(MouseEvent mouseEvent) {
		if (mouseEvent.getEventType() == MouseEvent.MOUSE_MOVED) {
			ControlPoint controlPoint = Tools.getControlPointAt(MainApp.canvas, MainApp.controlPoints,
					mouseEvent.getX(), mouseEvent.getY(), 0.0, CurveDrawer.DOT_SIZE / 2);
			MainApp.actualControlPoint = controlPoint;
			curveDrawer.accept(MainApp.controlPoints);
		}
		if (mouseEvent.getButton() == MouseButton.PRIMARY) {
			if (mouseEvent.getEventType() == MouseEvent.MOUSE_PRESSED) {
				ControlPoint point = Tools.getControlPointAt(MainApp.canvas, MainApp.controlPoints, mouseEvent.getX(),
